...
# Configuration
FUNCTION_NAME="pfb-leaderboard-submission"
ROLE_NAME="pfb-leaderboard-lambda-role"
REGION="us-east-1"
...
aws iam put-role-policy \
  --role-name $ROLE_NAME \
  --policy-name DynamoDBLeaderboardPolicy \
  --policy-document '{
    "Version": "2012-10-17",
    "Statement": [
      {
...
aws lambda create-function \
  ...
  --handler lambda-leaderboard-submission.handler \
  --zip-file fileb://lambda-leaderboard-submission.js \
  --region $REGION \
  --timeout 30 \
...
echo ""
echo "🔧 Next steps:"
echo "1. Test the function with: node test-lambda-leaderboard.js"
echo "2. Update the app to call this Lambda function" 