...
async function fixScoreType() {
  console.log('🔧 Fixing score type to return integers...\n');
  
  // The corrected code that returns integer scores
  ...
  try {
    ...
    if (action === "submitGameResult") {
      ...
      return {
        ...
        rankings: [{
          rank: 1,
          username: username,
          score: Math.round(score), // Convert to integer
          guesses: guesses,
          hints: hints,
    ...
    if (action === "getLeaderboard") {
      ...
      return (result.Items || []).map((item, index) => ({
        ...
        score: Math.round(item.score), // Convert to integer
